  ...
  return (
    <>
      <div className="fixed top-0 left-0 right-0 z-50 flex h-16 items-center justify-between border-b border-[#d3dee2] bg-[#f2f5f6] px-4 lg:hidden">
        ...
        <button onClick={() => setIsMobileOpen((prev) => !prev)} className="rounded-lg p-2 text-[#32505a] transition-colors hover:bg-[#e0eaed]" aria-label="Abrir menu lateral" type="button">
          {isMobileOpen ? <X size={24} /> : <Menu size={24} />}
        </button>
      </div>

      {isMobileOpen && <div className="fixed inset-0 z-40 bg-black/45 lg:hidden" onClick={() => setIsMobileOpen(false)} />}

      <aside className={`fixed left-0 z-40 h-full border-r border-[#d3dee2] bg-[#f2f5f6] transition-all duration-300 ${isCollapsed ? "w-20" : "w-64"} lg:top-0 ${isMobileOpen ? "top-16 h-[calc(100vh-4rem)]" : "top-16 h-[calc(100vh-4rem)] -translate-x-full lg:translate-x-0"}`}>
        <div className="flex h-full flex-col overflow-hidden">
          <div className="hidden border-b border-[#d3dee2] p-4 lg:block">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div className="overflow-hidden">
                <Image src={sociosaLogo} alt="SocioSA" width={140} height={40} className={`w-auto shrink-0 ${isCollapsed ? "h-8" : "h-10"}`} priority />
              </div>
              <button onClick={() => setIsCollapsed((prev) => !prev)} className="rounded-lg p-1 text-[#48636b] transition-colors hover:bg-[#dde8eb]" aria-label="Colapsar menu" type="button">
          ...
          <nav className="flex-1 overflow-y-auto py-4">
            <ul className="space-y-1 px-3">
              {menuItems.map((item) => {
                ...
                return (
                  <li key={item.id}>
                    <button onClick={() => handleNavigate(item.id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 transition-all ${isActive ? "bg-linear-to-r from-[#007c98] to-[#0a6c84] text-white shadow-[0_8px_18px_rgba(0,124,152,0.22)]" : isCoraShortcut ? "text-[#8f63d9] hover:bg-[#ede8f8]" : "text-[#17343d] hover:bg-[#e2ecef]"}`} title={isCollapsed ? item.label : undefined} type="button">
              ...
              <li className="pt-3 lg:hidden">
                <button onClick={handleLogoutClick} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-[#dd3f62] transition-colors hover:bg-[#fff0c2]" type="button">
          ...
          <div className="hidden shrink-0 border-t border-[#d3dee2] bg-[#f2f5f6] p-3 lg:block lg:bg-transparent">
            <button onClick={handleLogoutClick} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-[#dd3f62] transition-colors hover:bg-[#fff0c2] lg:justify-start" title={isCollapsed ? "Cerrar sesion" : undefined} type="button">
